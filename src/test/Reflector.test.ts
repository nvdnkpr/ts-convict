...
export class ReflectorTester {
    ...
    public checkPropCount() {
        ...
        assert.strictEqual(
            props.length,
            4,
            "There should be exactly 4 properties"
        );
        expect(props).to.have.all.members([
            "name",
            "host",
            "port",
            "subConfig",
    ...
    public checkPropConvictMeta() {
        ...
        assert.deepStrictEqual(
            actual,
            expected,
            "The name property is not correct in the metadata"
        );
    }
    ...
    public testTheIsConstructor() {
        assert.strictEqual(
            reflect.isConstructor(WithMainParent),
            true,
            "The WithMainParent is a constructor though"
        );
        assert.strictEqual(
            reflect.isConstructor(parent),
            false,
            "The parent is an instance of WithMainParent, not a constructor"
        );
        assert.strictEqual(
            reflect.isConstructor(() => {}),
            false,
            "That is not a constructor, it's an arrow function"
        );
        assert.strictEqual(
            reflect.isConstructor(function () {}),
            false,
            "That is not a constructor, it's a function"
        );
        assert.strictEqual(
            reflect.isConstructor(function () {
                this.msg = "";
            ...
            false,
            "That is a constructor, it's a function with this, but this won't be caught"
        );
        assert.strictEqual(
            reflect.isConstructor(String),
            true,
            "That is a constructor, it's a String constructor"
        );
        assert.strictEqual(
            reflect.isConstructor("hecka"),
            false,
            "That is not a constructor, it's a String"
        );
        assert.strictEqual(
            reflect.isConstructor(NotRealConfigClass),
            true,
            "The NotRealConfigClass is a constructor"
        );
    }
    ...
    public testTheIsConfigClass() {
        assert.strictEqual(
            reflect.isConfigClass(WithMainParent),
            true,
            "The WithMainParent should be a config class"
        );
        assert.strictEqual(
            reflect.isConfigClass(parent),
            false,
            "The parent is an instance of a config class, not a config class"
        );
        assert.strictEqual(
            reflect.isConfigClass(parent.constructor),
            true,
            "The parent.constructor is a config class"
        );
        assert.strictEqual(
            reflect.isConfigClass(String),
            false,
            "A String constructor is not a config class"
        );
        assert.strictEqual(
            reflect.isConfigClass(NotRealConfigClass),
            false,
            "A NotRealConfigClass is not a config class"
        );
    }
    ...
    public testgettingClassSchema() {
        ...
        assert.deepStrictEqual(
            actual,
            expected,
            "The config schema has differed"
        );
    }
}
