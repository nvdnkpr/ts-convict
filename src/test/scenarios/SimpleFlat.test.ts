...
export class NamedTest {

    /**
     * Make a new one each and every test.
     */
    public before() {
        tsConvict = new TSConvict(Named);
    }
    ...
    public validConfig() {
        ...
        const myValidConfig: Named = tsConvict.load(myRawConfig);
        //make sure we got a proper serialized type back
        assert.strictEqual(
            (myValidConfig instanceof Named),
            true,
    ...
    public testGettingValidDefaultConfig() {
        const myValidConfig: Named = tsConvict.load();
        //make sure we got a proper serialized type back
        assert.strictEqual(
            (myValidConfig instanceof Named),
            true,
